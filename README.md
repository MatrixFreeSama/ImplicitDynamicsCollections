...
- `experiment/newton-reciprocal-alternation-dynamics-tournament-bare-c` keeps the same nonlinear structural-dynamics family but forces strict 1:1 reciprocal alternation; the measured operational envelopes show that alternation can widen, preserve, or shrink convergence depending on the solver pair and stage budget.
- `experiment/newton-information-deficiency-redirection-dynamics-bare-c` replaces unconditional alternation with a budget-aware Newton information gate and explicit specialist redirection; in the recorded 24-stage sweep Aitken redirection enlarges the Newton envelope while poorly matched or repeatedly unproductive specialists can consume the remaining convergence budget.
