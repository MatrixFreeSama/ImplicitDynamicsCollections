...
2. Newton performs a short sequence of correction updates toward the final implicit root.
...
The experiment does not force the Padé continuation all the way to h=1 on every relay stage. A declared fraction theta is used by scaling the coefficient sequence for U(theta t), t in [0,1], and then applying the same Chebyshev/Padé construction on that scaled interval.

This matters in the avalanche model. At drive 88, the full h=1 single-stage predictor had an original-equation residual of about 8.73e5 and Newton failed. A partial continuation can land in a useful Newton basin instead.
...
- theta = 0.7
- two Newton updates per relay stage
- at most 16 relay stages

The locked sweep shows a qualitatively different behavior from the single-stage hybrid. Pure old-state Newton fails at drive 78. The previous full-h single-stage NRII->Newton hybrid passes through drive 87 and fails at drive 88. The fixed alternating policy passes every point that was explicitly included in the locked sweep through drive 107. It fails at drive 108, passes drive 109, and fails at drive 110, showing that Padé/continuation safety is not monotone in the drive parameter.

Representative cases:

- drive 88: pure Newton fails; single-stage hybrid fails; fixed alternating passes in 9 relay stages / 17 Newton updates, residual 3.13e-13.
- drive 94: pure Newton fails; single-stage hybrid fails; fixed alternating passes in 6 relay stages / 11 Newton updates, residual 1.07e-13.
- drive 104: pure Newton fails; single-stage hybrid has no declared safe full-step seed; fixed alternating passes in 4 relay stages / 7 Newton updates, residual 1.90e-13.
- drive 107: fixed alternating passes in 4 relay stages / 7 Newton updates, residual 1.02e-12.
...
For each relay stage, every structurally safe NRII partial predictor is given two Newton updates. If none reaches the final tolerance, the Newton-corrected candidate with the smallest post-update residual becomes the next NRII anchor. This is the explicit Newton-to-NRII feedback step.
...
- drive 108: pass, 10 relay stages, 46 candidate trials, 92 Newton updates in all trials, residual 1.99e-13.
- drive 120: pass, 6 relay stages, 24 trials, residual 4.38e-13.
- drive 140: pass, 8 relay stages, 29 trials, residual 4.21e-13.
- drive 160: pass, 6 relay stages, 20 trials, residual 7.98e-13.
- drive 180: pass, 5 relay stages, 15 trials, residual 8.28e-13.
- drive 190: pass, 6 relay stages, 21 trials, residual 4.04e-12.
- drive 195: fail under the declared order/theta set; first accepted feedback stage remains at about 5.54e11 residual.
- drive 198: no structurally safe declared candidate was found.
- drive 200: fail under the declared policy.

Thus the reciprocal construction pushes the tested old-state root-basin stress far beyond both pure old-state Newton and the prior single-stage hybrid, but it still has a finite domain.
